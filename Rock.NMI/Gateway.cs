...
namespace Rock.NMI
{
    ...
    [BooleanField("NMI Vault Enabled", "If service is enabled on NMI, activating this option will save customer data on NMI's Vault.", false, "", 8, "NMIVaultEnabled")]
    public class Gateway : ThreeStepGatewayComponent
    {
        ...
        /// <summary>
        /// Gets a value indicated whether NMI Vault is Enabled.
        /// </summary>
        /// <value>
        /// <c>true</c> if NMI Vault should add customers to vault; otherwise, <c>false</c>.
        /// </value>
        public bool NMIVaultEnabled(FinancialGateway financialGateway)
        {

            using ( var rockContext = new RockContext() )
            {
                Guid financialGatewayGuid = EntityTypeGuids.FINANCIAL_GATEWAY.AsGuid();
                var financialGatewayEntityTypeId = new EntityTypeService( rockContext ).Queryable().Where( x => x.Guid == financialGatewayGuid ).Select( x => x.Id ).First().ToString().AsInteger();
                var NMIVaultEnableAttributeId = new AttributeService( rockContext ).Queryable().Where( x => x.EntityTypeId == financialGatewayEntityTypeId && x.Key == "NMIVaultEnabled" && x.EntityTypeQualifierValue == financialGateway.EntityTypeId.ToString() ).Select( x => x.Id ).First().ToString().AsInteger();
                var NMIVaultEnabledValue = new AttributeValueService( rockContext ).Queryable().Where( x => x.AttributeId == NMIVaultEnableAttributeId && x.EntityId == financialGateway.Id ).Select(x => x.Value).FirstOrDefault().ToString().AsBoolean();

                return NMIVaultEnabledValue;
            }  
        }
        ...
        public override string ChargeStep1( FinancialGateway financialGateway, PaymentInfo paymentInfo, out string errorMessage )
        {
            ...
            try
            {
                ...
                {
                    if ( !isReferencePayment && NMIVaultEnabled(financialGateway))
                    {
