...
var WillowRock = (function (wr) {
    ...
    wr.protectionApp = (function (pa) {
        ...
        pa.init = function (options) {
            ...
            pa.$validSSN = $('input[type=radio][name=validSSN]');
            ...
            pa.$maxLengthFields.keyup(function (e) {
                ...
                if (len < min) {
                    $cntr.text('Explique más por favor, le quedan solo ' + (min - len) + ' caracteres por escribir');
                } else {
            ...
            pa.$validSSN.change(function () {
                if (this.value == 'true')
                {
                  $('#validSSNDiv').show();
                  $('#ApplicantSsn').val('');
                  $('#ApplicantSsnVerify').val('');
                }
                else if (this.value == 'false')
                {
                  $('#validSSNDiv').hide();
                  $('#ApplicantSsn').val('111-11-1111');
                  $('#ApplicantSsnVerify').val('111-11-1111');
                }
            });
                        ...
                        if ($npane[0]) {
                            ...
                            if ($cpane.find('#chkCorrectInfo')[0]) {
                                if (!$('#chkCorrectInfo').is(':checked')) {
                                    a.appendError('Por favor confirma que la información proveida es correcta');
                                    $('#chkCorrectInfo').focus();
                                } else if (!$('#chkAuthReleaseInfo').is(':checked')) {
                                    a.appendError('Debe autorizar la divulgación de su información');
...
var WillowRockValidation = function (options) {
    ...
    vld.validateDate = function (val) {
        ...
        if (valid) {
            ...
            if (date > now)
                return ' No se permite una fecha en el futuro';
        }
    ...
    vld.validateInputField = function ($ctrl, val) {
        ...
        var addMsg = (val && val.length < min) ? 'Debe de teclear al menos ' + min + ' caracteres' : '';
        if (ctype == 'radio') {
                ...
                switch (ctrlId) {
                    ...
                    case "GuardianSignature":
                        if ($('#GuardianSignature').val().trim() == '') {
                            valid = false;
                            addMsg = "Se requiere la firma de un guardián legal";
                        }
                        break;
