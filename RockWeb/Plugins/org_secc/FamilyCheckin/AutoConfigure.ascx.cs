...
namespace RockWeb.Plugins.org_secc.FamilyCheckin
{
    ...
    public partial class AutoConfigure : CheckInBlock
    {
        ...
        private void NewSetKiosk( string ip )
        {
            var kioskName = ip;
            var rockContext = new RockContext();
            var kioskService = new KioskService( rockContext );
            ...
            try
            {
                ...
                kiosk = kioskService.GetByClientName( hostName );
                kioskName = hostName;
            }
            catch (SocketException)
            {
                // This IP has no Host Entry, so search by IP address
                kiosk = kioskService.GetByIPAddress( ip, true );
            }
        ...
        private void GetKioskType( Kiosk kiosk, RockContext rockContext )
        {
            ...
            {                
                ...
                Session["DeviceAutoConfigured"] = true;
            ...
            else
            {
                ...
                pnlMain.Visible = true;
                Session["DeviceAutoConfigured"] = false;
