...
namespace RockWeb.Plugins.org_willowcreek.Event
{
    /// <summary>
    /// Template block for developers to use to start a new block.
    /// </summary>
    [DisplayName( "Registration Instance Detail" )]
    [Category( "org_willowcreek > Event" )]
    ...
    [BooleanField( "Advanced Grid Filters", "Display advanced grid filters", false, "", 9 )]
    public partial class RegistrationInstanceDetail : Rock.Web.UI.RockBlock, IDetailBlock
    {
        ...
        private Rock.Reporting.FilterMode FilterMode;
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            if ( GetAttributeValue( "AdvancedGridFilters" ).AsBoolean() )
            {
                FilterMode = Rock.Reporting.FilterMode.AdvancedFilter;
            }
            else
            {
                FilterMode = Rock.Reporting.FilterMode.SimpleFilter;
            }
            ...
            gRegistrations.Actions.CopyAddButton( phRegistrationsGridActions.Controls, Page );
...
";
        ...
        protected void gRegistrations_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            ...
            if ( registration != null )
            {
                ...
                var lRegisteredByEmail = e.Row.FindControl( "lRegisteredByEmail" ) as Literal;
                var lRegisteredByMobile = e.Row.FindControl( "lRegisteredByMobile" ) as Literal;
                var mobilePhoneNumberDefinedValue = new DefinedValueService( new RockContext() ).Queryable().AsNoTracking().Where( d => d.Guid.ToString() == org.willowcreek.SystemGuid.DefinedValueGuids.MOBILE_PHONE ).FirstOrDefault();
                if ( lRegisteredBy != null && lRegisteredByEmail != null && lRegisteredByMobile != null )
                {
                    if ( registration.PersonAlias != null && registration.PersonAlias.Person != null )
                    {
                        lRegisteredBy.Text = registration.PersonAlias.Person.FullNameReversed;
                        lRegisteredByEmail.Text = registration.PersonAlias.Person.Email;
                        lRegisteredByMobile.Text = registration.PersonAlias.Person.PhoneNumbers.Where( p => p.NumberTypeValueId == mobilePhoneNumberDefinedValue.Id ).Select( p => p.NumberFormatted ).FirstOrDefault();
                    }
                    else
                    {
                        lRegisteredBy.Text = string.Format( "{0}, {1}", registration.LastName, registration.FirstName );
                        lRegisteredByEmail.Text = registration.ConfirmationEmail;
                    }
                }
                ...
                foreach ( var field in RegistrantFields )
                {
                    if ( field.FieldSource == RegistrationFieldSource.PersonField && field.PersonFieldType.HasValue )
                    {
                        switch ( field.PersonFieldType.Value )
                        {
                            ...
                            case RegistrationPersonFieldType.Anniversary:
                                {
                                    var drpAnniversaryFilter = phRegistrantFormFieldFilters.FindControl( "drpAnniversaryFilter" ) as DateRangePicker;
                                    if ( drpAnniversaryFilter != null )
                                    {
                                        fRegistrants.SaveUserPreference( "Anniversary", drpAnniversaryFilter.DelimitedValues );
                                    }
                    ...
                    if ( field.Attribute != null )
                    {
                        ...
                        if ( filterControl != null )
                        {
                            try
                            {
                                var values = attribute.FieldType.Field.GetFilterValues( filterControl, field.Attribute.QualifierValues, FilterMode );
                                fRegistrants.SaveUserPreference( attribute.Key, attribute.Name, attribute.FieldType.Field.GetFilterValues( filterControl, attribute.QualifierValues, FilterMode ).ToJson() );
                            }
                            catch { }
                        }
                    }
        ...
        protected void fRegistrants_DisplayFilterValue( object sender, GridFilter.DisplayFilterValueArgs e )
        {
            if ( RegistrantFields != null )
            {
                ...
                {
                    //If key and name do not match for non RegistrantFields (as it is a RegistrationPersonFieldType), clear the value for the following filters.
                    //This fixes issue of orphaned filters showing up on the grid as active when the fields are removed from the registration template. 
                    if ( e.Key != e.Name )
                    {
                        switch ( e.Key )
                        {
        ...
        private void gRegistrants_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            ...
            if ( registrant != null )
            {
                // Set the registrant name value (used in the registrants panel).
                var lRegistrantFirstName = e.Row.FindControl( "lRegistrantFirstName" ) as Literal;
                var lRegistrantLastName = e.Row.FindControl( "lRegistrantLastName" ) as Literal;
                if ( lRegistrantFirstName != null && lRegistrantLastName != null )
                {
                    if ( registrant.PersonAlias != null && registrant.PersonAlias.Person != null )
                    {
                        lRegistrantFirstName.Text = registrant.PersonAlias.Person.FirstName;
                        lRegistrantLastName.Text = registrant.PersonAlias.Person.LastName +
                    ...
                    {
                        lRegistrantFirstName.Text = string.Empty;
                        lRegistrantLastName.Text = string.Empty;
                    }
                }

                // Set the registrant name value (used in the group placements panel).
                ...
                //Set the address to grid
                var lAddress = e.Row.FindControl( "lAddress" ) as Literal;
                if ( lAddress != null )
                {
                    if ( registrant.PersonAlias != null )
                    {
                        ...
                        using ( var rockContext = new RockContext() )
                        {
                            Guid homeAddressTypeGuid = DefinedValueGuids.HOME_ADDRESS.AsGuid();
                            int? homeAddressType = new DefinedValueService( rockContext ).Queryable().Where( t => t.Guid == homeAddressTypeGuid ).Select( t => t.Id ).FirstOrDefault();
                            Guid familyTypeGuid = GroupTypeGuids.FAMILY.AsGuid();
                            int? familyType = new GroupTypeService( rockContext ).Queryable().Where( f => f.Guid == familyTypeGuid ).Select( f => f.Id ).FirstOrDefault();

                            var groups = new GroupMemberService( rockContext ).Queryable().Where( g => g.PersonId == registrant.PersonAlias.PersonId ).Select( g => g.GroupId ).ToList();
                            var familyGroup = new GroupService( rockContext ).Queryable().Where( f => groups.Contains( f.Id ) && f.GroupTypeId == familyType ).Select( f => f.Id );
                            var locations = new GroupLocationService( rockContext ).Queryable().Where( g => familyGroup.Contains( g.GroupId ) && g.GroupLocationTypeValueId == homeAddressType ).Select( g => g.LocationId ).ToList();
                            var homeAddress = new LocationService( rockContext ).Queryable().Where( h => locations.Contains( h.Id ) );

                            if ( homeAddress.Any() )
                            {
                                var addresses = new List<string>();
                                foreach ( Location location in homeAddress )
                                {
                                    addresses.Add( FormattAddress( location ) );
                                }

                                lAddress.Text = addresses.AsDelimited( "<br/>" );
                            }
                ...
                if ( _homeAddresses.Count > 0 )
                {
                    ...
                    var location = _homeAddresses.Where( a => a.Key == registrant.PersonId.Value ).FirstOrDefault().Value;
                ...
                // add phone numbers if exporting
                if ( _phoneNumbers.Count > 0 )
                {
                    using ( var rockContext = new RockContext() )
                    {
                        var lMobilePhone = e.Row.FindControl( "lMobilePhone" ) as Literal;
                        var lHomePhone = e.Row.FindControl( "lHomePhone" ) as Literal;
                        var lWorkPhone = e.Row.FindControl( "lWorkPhone" ) as Literal;
                        var dvIdMobile = new DefinedValueService( rockContext ).Queryable().Where( v => v.Guid.ToString() == DefinedValueGuids.MOBILE_PHONE ).Select( v => v.Id ).FirstOrDefault();
                        var dvIdHome = new DefinedValueService( rockContext ).Queryable().Where( v => v.Guid.ToString() == DefinedValueGuids.HOME_PHONE ).Select( v => v.Id ).FirstOrDefault();
                        var dvIdWork = new DefinedValueService( rockContext ).Queryable().Where( v => v.Guid.ToString() == DefinedValueGuids.WORK_PHONE ).Select( v => v.Id ).FirstOrDefault();
                        lMobilePhone.Text = _phoneNumbers.Where( p => p.NumberTypeValueId == dvIdMobile && p.PersonId == registrant.PersonId ).Select( p => p.NumberFormatted ).FirstOrDefault();
                        lHomePhone.Text = _phoneNumbers.Where( p => p.NumberTypeValueId == dvIdHome && p.PersonId == registrant.PersonId ).Select( p => p.NumberFormatted ).FirstOrDefault();
                        lWorkPhone.Text = _phoneNumbers.Where( p => p.NumberTypeValueId == dvIdWork && p.PersonId == registrant.PersonId ).Select( p => p.NumberFormatted ).FirstOrDefault();
                    }
                }
            }
        }
                ...
                foreach ( var field in RegistrantFields )
                {
                    if ( field.FieldSource == RegistrationFieldSource.PersonField && field.PersonFieldType.HasValue )
                    {
                        switch ( field.PersonFieldType.Value )
                        {
                            ...
                            case RegistrationPersonFieldType.Anniversary:
                                {
                                    var drpAnniversaryFilter = phWaitListFormFieldFilters.FindControl( "drpAnniversaryFilter" ) as DateRangePicker;
                                    if ( drpAnniversaryFilter != null )
                                    {
                                        fWaitList.SaveUserPreference( "WL-Anniversary", drpAnniversaryFilter.DelimitedValues );
                                    }
            ...
            if ( instanceId.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    if ( isExporting )
                    {
                        ...
                        var dvIdHome = new DefinedValueService( rockContext ).Queryable().Where( v => v.Guid.ToString() == DefinedValueGuids.HOME_PHONE ).Select( v => v.Id ).FirstOrDefault();
                        var dvIdWork = new DefinedValueService( rockContext ).Queryable().Where( v => v.Guid.ToString() == DefinedValueGuids.WORK_PHONE ).Select( v => v.Id ).FirstOrDefault();
                        _phoneNumbers = new PhoneNumberService( rockContext ).Queryable().Where( p => personIds.Contains( p.PersonId ) &&
                                                                                    ( p.NumberTypeValueId == dvIdMobile || p.NumberTypeValueId == dvIdHome || p.NumberTypeValueId == dvIdWork ) ).ToList();
                    }
                    ...
                    if ( RegistrantFields != null )
                    {
                                ...
                                case RegistrationPersonFieldType.Anniversary:
                                    {
                                        ...
                                        if ( drpAnniversaryFilter != null )
                                        {
                                            if ( drpAnniversaryFilter.LowerValue.HasValue )
                                            {
                                                qry = qry.Where( r =>
                                                    r.PersonAlias.Person.AnniversaryDate.HasValue &&
                                                    r.PersonAlias.Person.AnniversaryDate.Value >= drpAnniversaryFilter.LowerValue.Value );
                                            }
                                            if ( drpAnniversaryFilter.UpperValue.HasValue )
                                            {
                                                qry = qry.Where( r =>
                                                    r.PersonAlias.Person.AnniversaryDate.HasValue &&
                                                    r.PersonAlias.Person.AnniversaryDate.Value <= drpAnniversaryFilter.UpperValue.Value );
                                            }
                                        }
                                        break;
                                    }
                        ...
                        if ( registrantAttributes != null && registrantAttributes.Any() )
                        {
                            ...
                            foreach ( var attribute in registrantAttributes )
                            {
                                ...
                                if ( filterControl != null )
                                {
                                    var filterValues = attribute.FieldType.Field.GetFilterValues( filterControl, attribute.QualifierValues, FilterMode );
                ...
                foreach ( var field in RegistrantFields )
                {
                    if ( field.FieldSource == RegistrationFieldSource.PersonField && field.PersonFieldType.HasValue )
                    {
                        switch ( field.PersonFieldType.Value )
                        {
                            case RegistrationPersonFieldType.Campus:
                                {
                                    ...
                                    ddlCampus.SetValue( fRegistrants.GetUserPreference( "Home Campus", "Home Campus" ) );
                                    ...
                                    ddlCampus2.SetValue( fRegistrants.GetUserPreference( "WL-Home Campus", "WL-Home Campus" ) );
                            ...
                            case RegistrationPersonFieldType.Address:
                                {
                                    ...
                                    addressField2.ID = "lAddress";
                                    addressField2.HeaderText = "Address";
                                    addressField2.ExcelExportBehavior = ExcelExportBehavior.NeverInclude;
                            ...
                            case RegistrationPersonFieldType.Email:
                                {
                                    ...
                                    tbEmailFilter.Text = fRegistrants.GetUserPreference( "Email", "Email" );
                                    ...
                                    tbEmailFilter2.Text = fRegistrants.GetUserPreference( "WL-Email", "WL-Email" );
                            ...
                            case RegistrationPersonFieldType.Birthdate:
                                {
                                    ...
                                    drpBirthdateFilter.DelimitedValues = fRegistrants.GetUserPreference( "Birthdate Range", "Birthdate Range" );
                                    ...
                                    drpBirthdateFilter2.DelimitedValues = fRegistrants.GetUserPreference( "WL-Birthdate Range", "WL-Birthdate Range" );
                            ...
                            case RegistrationPersonFieldType.Anniversary:
                                {
                                    var drpAnniversaryFilter = new DateRangePicker();
                                    drpAnniversaryFilter.ID = "drpAnniversaryFilter";
                                    drpAnniversaryFilter.Label = "Anniversary";
                                    drpAnniversaryFilter.DelimitedValues = fRegistrants.GetUserPreference( "Anniversary", "Anniversary" );
                                    phRegistrantFormFieldFilters.Controls.Add( drpAnniversaryFilter );

                                    var drpAnniversaryFilter2 = new DateRangePicker();
                                    drpAnniversaryFilter2.ID = "drpAnniversaryFilter";
                                    drpAnniversaryFilter2.Label = "Anniversary";
                                    drpAnniversaryFilter2.DelimitedValues = fRegistrants.GetUserPreference( "WL-Anniversary", "WL-Anniversary" );
                                    phWaitListFormFieldFilters.Controls.Add( drpAnniversaryFilter2 );

                                    string dataFieldExpression = "PersonAlias.Person.AnniversaryDate";
                                    var AnniversaryField = new DateField();
                                    AnniversaryField.DataField = dataFieldExpression;
                                    AnniversaryField.HeaderText = "Anniversary";
                                    AnniversaryField.SortExpression = dataFieldExpression;
                                    gRegistrants.Columns.Add( AnniversaryField );

                                    var AnniversaryField2 = new DateField();
                                    AnniversaryField2.DataField = dataFieldExpression;
                                    AnniversaryField2.HeaderText = "Anniversary";
                                    AnniversaryField2.SortExpression = dataFieldExpression;
                                    gGroupPlacements.Columns.Add( AnniversaryField2 );

                                    var AnniversaryField3 = new DateField();
                                    AnniversaryField3.DataField = dataFieldExpression;
                                    AnniversaryField3.HeaderText = "Anniversary";
                                    AnniversaryField3.SortExpression = dataFieldExpression;
                                    gWaitList.Columns.Add( AnniversaryField3 );
                            ...
                            case RegistrationPersonFieldType.Grade:
                                {
                                    ...
                                    var gradeUserPreference = fRegistrants.GetUserPreference( "Grade", "Grade" ).AsIntegerOrNull();
                                    ...
                                    var wlGradeUserPreference = fRegistrants.GetUserPreference( "WL-Grade", "WL-Grade" ).AsIntegerOrNull();
                            ...
                            case RegistrationPersonFieldType.Gender:
                                {
                                    ...
                                    ddlGenderFilter.SetValue( fRegistrants.GetUserPreference( "Gender", "Gender" ) );
                                    ...
                                    ddlGenderFilter2.SetValue( fRegistrants.GetUserPreference( "WL-Gender", "WL-Gender" ) );
                            ...
                            case RegistrationPersonFieldType.MaritalStatus:
                                {
                                    ...
                                    ddlMaritalStatusFilter.SetValue( fRegistrants.GetUserPreference( "Marital Status", "Marital Status" ) );
                                    ...
                                    ddlMaritalStatusFilter2.SetValue( fRegistrants.GetUserPreference( "WL-Marital Status", "WL-Marital Status" ) );
                            ...
                            case RegistrationPersonFieldType.MobilePhone:
                                {
                                    ...
                                    tbPhoneFilter.Text = fRegistrants.GetUserPreference( "Phone", "Phone" );
                                    ...
                                    tbPhoneFilter2.Text = fRegistrants.GetUserPreference( "WL-Phone", "WL-Phone" );
                    ...
                    else if ( field.Attribute != null )
                    {
                        ...
                        var control = attribute.FieldType.Field.FilterControl( attribute.QualifierValues, "filter_" + attribute.Id.ToString(), false, FilterMode );
                        if ( control != null )
                        {
                            ...
                            string savedValue = fRegistrants.GetUserPreference( attribute.Key, attribute.Name );
        ...
        private string FormattAddress( Location address )
        {
            ...
            string result = string.Format( "{0} {1} {2}, {3} {4}, {5}",
                address.Street1, address.Street2, address.City, address.State, address.PostalCode, address.Country ).ReplaceWhileExists( "  ", " " );
