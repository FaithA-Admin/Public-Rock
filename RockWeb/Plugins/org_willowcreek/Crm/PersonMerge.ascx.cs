...
", hfSelectedColumn.ClientID );
        ...
        private string GetValuesColumnHeader( int personId )
        {
            ...
            foreach ( var family in families )
            {
                ...
                var attributes = family.GetAuthorizedAttributes( Authorization.VIEW, CurrentPerson );
                var values = family.AttributeValues.Where( x => !string.IsNullOrWhiteSpace( x.Value.Value ) ).ToList();

                var displayAttributes = ( from a in attributes
                                          join v in values on a.Key equals v.Key
                                          select string.Format("<br><span>{0}: {1}</span>", a.Value, v.Value) );
                foreach (var da in displayAttributes)
                {
                    sbHeaderData.Append( da );
                }
