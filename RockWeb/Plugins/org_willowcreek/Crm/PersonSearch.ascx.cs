...
namespace RockWeb.Plugins.org_willowcreek.Crm
{
    ...
    public partial class PersonSearch : Rock.Web.UI.RockBlock
    {
        ...
        private DefinedValueCache _referenceStatus = null;
                ...
                if (person != null)
                {
                    ...
                    if (!person.IsBusiness)
                    {
                        ...
                        foreach (var location in person.HomeAddresses)
                        {
                            ...
                            {
                                sbPersonDetails.Append(string.Format("<small><br>{0}<br>{1} {2}, {3} {4}</small>", location.Street1, location.Street2, location.City, location.State, location.PostalCode));
                            }
                        }

                        // Add phone numbers, put Home first
                        sbPersonDetails.Append( "<br><small>" );
                        sbPersonDetails.Append( string.Join( "<br>", person.PhoneNumbers.OrderBy( x => x.NumberTypeValueId == 13 ? 0 : 1 ).ThenBy( x => x.NumberTypeValueId ).Select( phone => string.Format( "<nobr>{0}: {1}</nobr>", phone.NumberTypeValue, phone.ToString() ) ) ));
                        sbPersonDetails.Append( "</small>" );
            ...
            if (!string.IsNullOrWhiteSpace(type) && !string.IsNullOrWhiteSpace(term))
            {
                ...
                switch (type.ToLower())
                {
                    ...
                    case ("familyid"):
                        {
                            if (term.Contains("-"))
                                term = term.Substring(term.LastIndexOf("-") + 2);
                            people = org.willowcreek.Search.Common.FindPeopleByFamilyId(term);
                            break;
                        }
                }
                ...
                _referenceStatus = DefinedValueCache.Read("0FA8D32F-492A-478F-8692-012BBC0B004C");

                var peopleList = people.ToList();

                var personList = peopleList.Select( p =>
                 {
                     p.LoadAttributes();
                     return new PersonSearchResult
                     {
                         ...
                         BirthDay = p.BirthDay,
                         FamilyId = p.GetAttributeValue( "FamilyIdCard" ),
                         Badges = p.GetBadgeDisplay( 16, false ),
    ...
    public class PersonSearchResult
    {
        ...
        /// <summary>
        /// Gets or sets the Family Card Id.
        /// </summary>
        /// <value>The Family Card Id.</value>
