    ...
    public partial class ParentLocationEdit : RockBlock
    {
        ...
        private void GetData()
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                var dataSource = groupTypes.Select( t => new
                {
                    t.Id,
                    t.Name,
                    ParentLocations = t.AttributeValues["CheckInConfigParentLocations"]
        ...
        protected void dlgEdit_SaveClick( object sender, EventArgs e )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                config.SetAttributeValue( "CheckInConfigParentLocations", tbLocations.Text );
                config.SaveAttributeValue( "CheckInConfigParentLocations", rockContext );
            }
