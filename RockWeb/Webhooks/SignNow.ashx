        ...
        if ( signNowComponent != null )
        {
            if ( signNowData.meta.event_name == "document.update" )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    //Only save a document when the binary file doesnt exist otherwise duplicates could be created if SignNow API resends the document multiple times.
                    if ( document != null && document.BinaryFileId.HasValue == false)
                    {
                        ...
                        {
                            rockContext.SaveChanges(); //Save changes only if documentPath is valid.
