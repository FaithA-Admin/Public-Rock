...
public class Mandrill : IHttpHandler
{
            ...
            foreach ( var item in payload )
            {
                ...
                if ( item.EventType == MandrillEventType.HardBounced )
                {
                    ...
                    if ( !string.IsNullOrEmpty( item.Msg.Email ) )
                    {
                        ...
                        NotifySenderOfBounce( item.Msg.Email, item.Msg.Metadata["communication_recipientName"], item.Msg.Metadata["communication_sender"], item.Msg.Metadata["communication_subject"], item.EventDateTime, context );
                    }
                }
            }
        }
    ...
    public static void NotifySenderOfBounce( string recipient, string recipientName, string sender, string subject, DateTime bouncedDateTime, HttpContext context )
    {
        ...
        string bounceSenderEmailAddress = globalAttributes.GetValue( "OrganizationEmail" );
        string internalApplicationRoot = globalAttributes.GetValue( "InternalApplicationRoot" );
        if ( !string.IsNullOrEmpty( recipient ) && !string.IsNullOrEmpty( sender ) && !string.IsNullOrEmpty( bounceSenderEmailAddress ) && sender != bounceSenderEmailAddress )
            using ( var rockContext = new RockContext() )
            {
                ...
                try
                {
                    ...
                    Rock.Communication.Email.Send(bounceSenderEmailAddress,"Bounce Notification Service", "Unable to deliver email to: " + recipient, recipients, message.ToString(), "","", null, true );
                }
                catch ( Exception ex )
                {
                ...
                {
                    notifiedPeople.Append( "<a href=\"" + internalApplicationRoot + "Person/" + person.Id.ToString() + "\" target=\"_blank\">" + person.FullName + "</a>, " );
                }
                ...
                foreach ( var person in recipientPeopleWithEmail )
                {
                    historyService.Add( new History
                    {
                        ...
                        EntityId = person.Id,
                        Summary = ( "The following email address: <span class='field-value'>" + recipient + "</span> couldn't be reached when sending an email with the following subject: <span class='field-value'>" + subject + "</span>.<br>The following " + (recipientPeopleWithEmail.Count()>1 ? "people were":"person was" ) + " notified about the bounce: " + (notifiedPeople.ToString().Substring( 0, notifiedPeople.ToString().Length - 3 )) )
                    } );
                }
                rockContext.SaveChanges();
            }
