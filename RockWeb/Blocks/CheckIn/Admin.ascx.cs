...
namespace RockWeb.Blocks.CheckIn
{
    ...
    public partial class Admin : CheckInBlock
    {
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                ", this.Page.ClientScript.GetPostBackEventReference( lbRefresh, "" ) );
                    ...
                    Guid hybridKioskDeviceType = Rock.SystemGuid.DefinedValue.DEVICE_TYPE_HYBRID_CHECKIN.AsGuid();
                    ddlKiosk.Items.Clear();
                    using ( var rockContext = new RockContext() )
                    {
                        ddlKiosk.DataSource = new DeviceService( rockContext )
                            .Queryable().AsNoTracking()
                            .Where( d => ( d.DeviceType.Guid.Equals( kioskDeviceType ) || d.DeviceType.Guid.Equals( hybridKioskDeviceType ) ) )
                            .OrderBy( d => d.Name )
                            .Select( d => new
                            {
                                d.Id,
        ...
        private void AttemptKioskMatchByIpOrName()
        {
            ...
            var hybridKioskDeviceTypeId = DefinedValueCache.Read( Rock.SystemGuid.DefinedValue.DEVICE_TYPE_HYBRID_CHECKIN ).Id;
            using ( var rockContext = new RockContext() )
            {
                ...
                //If its not a kiosk try a hybrid device type.
                if ( device == null )
                {
                    device = new DeviceService( rockContext ).GetByIPAddress( Rock.Web.UI.RockPage.GetClientIpAddress(), hybridKioskDeviceTypeId, !enableReverseLookup );
                }
                if ( device != null )
                {
                    ClearMobileCookie();
...
", this.Page.ClientScript.GetPostBackEventReference( lbCheckGeoLocation, "" ) );
        ...
        public static Device GetCurrentKioskByGeoFencing( string sLatitude, string sLongitude )
        {
            ...
            var checkInDeviceTypeId = DefinedValueCache.Read( Rock.SystemGuid.DefinedValue.DEVICE_TYPE_CHECKIN_KIOSK ).Id;
            var hybridKioskDeviceType = DefinedValueCache.Read( Rock.SystemGuid.DefinedValue.DEVICE_TYPE_HYBRID_CHECKIN ).Id;
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( kiosk == null )
                {
                    kiosk = new DeviceService( rockContext ).GetByGeocode( latitude, longitude, hybridKioskDeviceType );
                }
                return kiosk;
            }
        }
