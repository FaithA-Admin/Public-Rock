...
namespace RockWeb.Blocks.CheckIn
{
    ...
    public partial class Welcome : CheckInBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            //Set the theme if device is a kiosk (auto configured).
            var pageRef = RockPage.PageReference;
            string theme = string.Empty;
            if ( Session["CheckinTypeId"] != null && Session["DeviceAutoConfigured"] != null )
            {
                var checkinTypeId = ( int ) Session["CheckinTypeId"];
                var deviceAutoConfigured = ( bool ) Session["DeviceAutoConfigured"];
                if ( deviceAutoConfigured )
                {
                    btnReloadKioskConfiguration.Visible = true;
                    btnReloadKioskConfiguration.Style.Value = "background-color:Transparent;border-color:Transparent;border-style:solid;width:150px;";

                    using ( var rockContext = new RockContext() )
                    {
                        var checkInThemeAttribute = AttributeCache.Read( Rock.SystemGuid.Attribute.CHECKIN_THEME.AsGuid(), rockContext );
                        var checkInThemeAttributeId = new AttributeService( rockContext ).Queryable().Where( k => k.Guid == checkInThemeAttribute.Guid ).Select( k => k.Id ).FirstOrDefault();
                        theme = new AttributeValueService( rockContext ).Queryable().Where( i => i.AttributeId == checkInThemeAttributeId && i.EntityId == checkinTypeId ).Select( v => v.Value ).FirstOrDefault();
                    }
                    if ( ( pageRef.QueryString["theme"].IsNullOrWhiteSpace() || pageRef.QueryString["theme"] != theme ) && ( Session["CheckinTypeId"] != null && !Session["CheckinTypeId"].ToString().IsNullOrWhiteSpace() ) )
                    {
                ...
                {
                    btnReloadKioskConfiguration.Visible = false;
                }              
            }


            if ( string.IsNullOrWhiteSpace( lHostName.Text ) )
            {
                var ip = Rock.Web.UI.RockPage.GetClientIpAddress();
                try
                {
                    lHostName.Text = Dns.GetHostEntry( ip ).HostName.ToLower();
                }
...
", CurrentTheme, CurrentKioskId, CurrentCheckinTypeId, CurrentGroupTypeIds.AsDelimited( "," ) );
        ...
        /// <summary>
        /// Handles the Click event of the btnChangeCheckinConfiguration control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnEnglishCheckinConfiguration_click(object sender, EventArgs e )
        {
            changeCheckinConfiguration( false );
        }
        ...
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnSpanishCheckinConfiguration_click( object sender, EventArgs e )
        {
            changeCheckinConfiguration( true );
        }

        private void changeCheckinConfiguration(bool spanish )
        {
            var checkinConfiguationURL = Request.Url.Scheme + System.Uri.SchemeDelimiter + Request.Url.Host + ( Request.Url.IsDefaultPort ? "" : ":" + Request.Url.Port ) + "/checkin";
            if ( spanish )
            {
                checkinConfiguationURL += "_es";
            }

            if ( checkinConfiguationURL != Request.Url.PathAndQuery )
            {
                Response.Redirect( checkinConfiguationURL, false );
                Context.ApplicationInstance.CompleteRequest();
            }
        }

        /// <summary>
        /// Handles the Click event of the btnReloadKioskConfiguration control.
        /// </summary>
        /// <param name="sender">The souces of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnReloadKioskConfiguration_Click( object sender, EventArgs e )
        {
            if( Session["DeviceAutoConfigured"] != null  && ( bool ) Session["DeviceAutoConfigured"] )
            {   
                var checkIn = Request.Url.LocalPath.Split( '/' );

                switch ( checkIn[1].ToLower() )
                {
                    case "checkin":
                        changeCheckinConfiguration( false );
                        break;
...
", this.Page.ClientScript.GetPostBackEventReference( lbRefresh, "" ) );
        ...
        private void RefreshView()
        {
            ...
            if ( KioskType != null && !KioskType.IsOpen() )
            {
