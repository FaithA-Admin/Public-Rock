...
namespace RockWeb.Blocks.Groups
{
    ...
    [BooleanField( "Load All Groups By Default", "", false, "CustomSetting" )]
    [BooleanField( "Display Campus Filter", "", false, "CustomSetting" )]
    [BooleanField( "Enable Campus Context", "", false, "CustomSetting" )]
    [BooleanField( "Display Searchbox", "", false, "CustomSetting" )]
    ...
    [BooleanField( "Allow Zoom On Map", "", false, "CustomSetting" )]
    [BooleanField( "Hide Real Group Location", "", false, "CustomSetting" )]
...
{% if LinkedPages.RegisterPage and LinkedPages.RegisterPage != '' %}
    {% if LinkedPages.RegisterPage contains '?' %}
        <a class='btn btn-xs btn-action' href='{{ LinkedPages.RegisterPage }}&GroupGuid={{ Group.Guid }}'>Contact Leader</a>
    {% else %}
        <a class='btn btn-xs btn-action' href='{{ LinkedPages.RegisterPage }}?GroupGuid={{ Group.Guid }}'>Contact Leader</a>
    {% endif %}
{% endif %}
...
", "CustomSetting" )]
    ...
    public partial class GroupFinder : RockBlockCustomSettings
    {
        ...
        protected void lbSave_Click( object sender, EventArgs e )
        {
            ...
            SetAttributeValue( "LoadAllGroupsByDefault", cbLoadAllGroupsByDefault.Checked.ToString() );
            ...
            SetAttributeValue( "DisplaySearchbox", cbSearchbox.Checked.ToString() );
            ...
            SetAttributeValue( "AllowZoomOnMap", cbAllowZoomOnMap.Checked.ToString() );
            SetAttributeValue( "HideRealGroupLocation", cbHideRealGroupLocation.Checked.ToString() );
        ...
        protected override void ShowSettings()
        {
            ...
            cbLoadAllGroupsByDefault.Checked = GetAttributeValue( "LoadAllGroupsByDefault" ).AsBoolean();
            ...
            cbSearchbox.Checked = GetAttributeValue( "DisplaySearchbox" ).AsBoolean();

            cbShowMap.Checked = GetAttributeValue( "ShowMap" ).AsBoolean();
            cbAllowZoomOnMap.Checked = GetAttributeValue( "AllowZoomOnMap" ).AsBoolean();
            cbHideRealGroupLocation.Checked = GetAttributeValue( "HideRealGroupLocation" ).AsBoolean();
        ...
        private void ShowView()
        {
            ...
            else
            {
                ...
                string scheduleFilters = GetAttributeValue( "ScheduleFilters" );
                if ( !string.IsNullOrWhiteSpace( scheduleFilters ) || AttributeFilters.Any() || txtSearch.Visible )
                {
                    phFilterControls.Visible = true;
                    btnSearch.Visible = true;
                }
            ...
            if ( pnlResults.Visible || GetAttributeValue( "LoadAllGroupsByDefault" ).AsBoolean() )
            {
                ShowResults();
            }
        }
        ...
        private void BuildDynamicControls()
        {
            ...
            if ( GetAttributeValue( "DisplaySearchbox" ).AsBoolean() )
            {
            ...
            if ( registerPage.PageId > 0 )
            {
                ...
                registerColumn.ToolTip = "Contact Leader";
                registerColumn.HeaderText = "Contact Leader";
        ...
        private void ShowResults()
        {
            ...
            //Filter group by name or description
            if ( GetAttributeValue( "DisplaySearchbox" ).AsBoolean() && !txtSearch.Text.IsNullOrWhiteSpace() )
            {
                groupQry = groupQry.Where( w => w.Name.Contains( txtSearch.Text ) || w.Description.Contains( txtSearch.Text ) );
            }
...
", personLocation.FormattedHtmlAddress );
                ...
                if ( gGroups.SortProperty == null && showProximity && GetAttributeValue( "SortByDistance" ).AsBoolean() && distances.Count > 0 )
                {
                ...
                if ( showMap && groups.Any() )
                {
                    ...
                    foreach ( var gl in groupLocations )
                    {
                        ...
                        if ( group != null )
                        {
                            ...
                            bool hideRealGroupLocation = GetAttributeValue( "HideRealGroupLocation" ).AsBoolean();
                            var mapItem = new FinderMapItem( gl.Location );
                            mapItem.EntityTypeId = EntityTypeCache.Read( "Rock.Model.Group" ).Id;
                            mapItem.EntityId = group.Id;
                            mapItem.Name = group.Name;
                            if ( hideRealGroupLocation && mapItem.Point.Latitude.HasValue && mapItem.Point.Longitude.HasValue && !IsSafeLocationToShowOnMap(gl.Location) )
                            {
                                mapItem.Point.Latitude = Math.Round( mapItem.Point.Latitude.Value, 2 );
                                mapItem.Point.Longitude = Math.Round( mapItem.Point.Longitude.Value, 2 );
                            }
        ...
        {
            pnlMap.Visible = true;
            bool allowZoomOnMap = GetAttributeValue( "AllowZoomOnMap" ).AsBoolean();
...
";
            ...
            string mapScript = string.Format( mapScriptFormat,
                ...
                longitude,          // 8
                zoom,               // 9
                ( !allowZoomOnMap ).ToString().ToLower(), //10
                ( allowZoomOnMap ).ToString().ToLower() //11
                );
        ...
        private bool IsSafeLocationToShowOnMap(Location location)
        {
            //Check for campus locations  
            using ( var rockContext = new RockContext() )
            {
                var campusesLocationsIds = new CampusService( rockContext ).Queryable().Select( l => l.LocationId ).ToList();
                var campusesLocations = new LocationService( rockContext ).Queryable().Where( l => campusesLocationsIds.Contains( l.Id ) ).ToList();

                foreach(var campusLocation in campusesLocations )
                {
                    if(location.Street1 == campusLocation.Street1 && location.City == campusLocation.City && location.State == campusLocation.State && location.Country == campusLocation.Country )
                    {
                        return true;
                    }
                }
