            ...
            if ( Page.IsValid && _isValidSettings )
            {
                ...
                if ( !IsSimple )
                {
                    ...
                    if ( IsFullWithSpouse && !string.IsNullOrWhiteSpace(tbSpouseFirstName.Text) && !string.IsNullOrWhiteSpace(tbSpouseLastName.Text) )
                    {
                        ...
                        History.EvaluateChange( changes, "Birthdate", spouse.BirthDate, bpSpouseBirthdate.SelectedDate );

                        History.EvaluateChange( changes, "Email", spouse.Email, tbSpouseEmail.Text );
