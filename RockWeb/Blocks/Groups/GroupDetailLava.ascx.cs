...
namespace RockWeb.Blocks.Groups
{
    ...
    [BooleanField( "Enable Group Name Edit", "Set this to true to enable the edit box for group 'Name'.", false, key: "EnableGroupNameEdit", order:17 )]
    [BooleanField( "Hide Group Schedule Edit", "Set this to true to hide the edit group schedule panel.", false, key: "HideGroupScheduleEdit", order:18 )]
    [BooleanField( "Hide Group Attributes Edit", "Set this to true to hide the edit of group attributes.", false, key: "HideGroupAttributesEdit", order: 19 )]
    [BooleanField( "Restrict block access only to group leaders and administrators.", "Set this to true if only group leaders and administrators should access this block.", false, key: "RestrictedAccessToLeadersOnly", order: 20)]
    public partial class GroupDetailLava : Rock.Web.UI.RockBlock
    {
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            if ( IsEditingGroup == true )
            {
                ...
                if ( !this.GetAttributeValue( "HideGroupAttributesEdit" ).AsBooleanOrNull() ?? false )
                {
            ...
            if ( IsEditingGroupMember == true )
            {
                ...
                //Exclude small group group-member attribute dates.
                List<string> excludedAttributes = new List<string>() { "PendingDate", "InactiveDate", "ActiveDate" };
                Rock.Attribute.Helper.AddEditControls( groupMember, phGroupMemberAttributes, true, string.Empty, excludedAttributes, true );
            }
        }
        ...
        private void BlockSetup()
        {
            ...
            bool enableNameEdit = this.GetAttributeValue( "EnableGroupNameEdit" ).AsBooleanOrNull() ?? false;
            if( enableNameEdit )
            {
        ...
        private void DisplayViewGroup()
        {
            if ( _groupId > 0 )
            {
                ...
                bool restrictedAccessToLeadersOnly = this.GetAttributeValue( "RestrictedAccessToLeadersOnly" ).AsBooleanOrNull() ?? false;
                GroupTypeRole currentPersonGroupRole = group.Members.Where( m => m.PersonId == CurrentPerson.Id ).Select( s => s.GroupRole ).FirstOrDefault();
                //Check if only leaders should have access to the group toolbox.
                if(restrictedAccessToLeadersOnly)
                {
                    if( (currentPersonGroupRole != null && currentPersonGroupRole.IsLeader) || UserCanAdministrate )
                    {
        ...
        private void DisplayEditGroup()
        {
            ...
            if ( _groupId != -1 )
            {
                ...
                if ( group.IsAuthorized( Authorization.EDIT, CurrentPerson ) )
                {
                    ...
                    SetScheduleControls( GroupTypeCache.Read(group.GroupTypeId), group );

                    if( this.GetAttributeValue( "HideGroupScheduleEdit" ).AsBooleanOrNull() ?? false )
                    {
                        pnlSchedule.Visible = false;
                    }
                }
