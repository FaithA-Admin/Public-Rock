...
";
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( Person != null && Person.Id != 0 )
            {
                ...
                if ( Person.GetFamilyRole() != null )
                {
                    if ( Person.GetFamilyRole().ToString() == "Adult" )
                    {
                        hbRelationshipType.Text = Person.FullName + " is an Adult, so for check-in relationships you probably want to use \"Can Check In\" and choose the person you want " + Person.NickName + " to check-in below. Otherwise, use \"Allow Check-In By\" and select the person that you want to be able to check " + Person.NickName + " in.";
                    }
                    else
                    {
                        hbRelationshipType.Text = Person.FullName + " is a Child, so for check-in relationships you probablyl want to use \"Allow Check-In By\" and select the person that you want to be able to check " + Person.NickName + " in. Otherwise, use \"Can Check In\" and choose the person you want " + Person.NickName + " to check-in below.";
                    }
                    //hbRelationshipType.Text = Person.GetFamilyRole().ToString() + " If " + Person.FullName + " is an adult, use \"Can Check In\" and choose the child below.  If " + Person.FullName + " is a child, use \"Allow Check-In By\" and choose the adult below.";
                }
