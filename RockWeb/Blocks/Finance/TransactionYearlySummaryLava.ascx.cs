    ...
    public partial class TransactionYearlySummaryLava : RockBlock, ISecondaryBlock
    {
        ...
        private void BindGrid()
        {
            ...
            if ( contributionType != null )
            {
                ...
                var financialAccounts = new FinancialAccountService( rockContext ).Queryable().Select( a => new { a.Id, Account = a } ).ToDictionary( k => k.Id, v => v );
                ...
                foreach ( var item in summaryList.GroupBy( a => a.Year ) )
                {
                    ...
                    foreach ( var a in item )
                    {
                        ...
                        accountDictionary.Add( "Account", financialAccounts.ContainsKey( a.AccountId ) ? financialAccounts[a.AccountId].Account.Name : string.Empty );
                        accountDictionary.Add( "AccountPublicName", financialAccounts.ContainsKey( a.AccountId ) ? financialAccounts[a.AccountId].Account.PublicName : string.Empty );
                        accountDictionary.Add( "AccountDescription", financialAccounts.ContainsKey( a.AccountId ) ? financialAccounts[a.AccountId].Account.Description : string.Empty );
                        accountDictionary.Add( "TotalAmount", a.TotalAmount );
                        accountDictionary.Add( "Count", a.Count );
