...
namespace RockWeb.Blocks.Finance
{
    ...
    [BooleanField( "Hide Account Controls", "If all unmatched transactions already have their amounts populated by a check scanner, account controls are not necessary " )]
    [BooleanField( "Show Legal Name", "Should the person picker show the full legal name (first, middle, last, suffix) rather than using the nickname?" )]
...
";
            ...
            ppSelectNew.UseLegalName = GetAttributeValue( "ShowLegalName" ).AsBoolean();
        }
        ...
        {
            if ( this.GetAttributeValue( "HideAccountControls" ).AsBoolean() )
            {
                rcwAccountSplit.Visible = false;
            }
            else
            {
                rcwAccountSplit.Visible = true;
        ...
        protected void btnNext_Click( object sender, EventArgs e )
        {
            ...
            // Commenting this section out as unnecessary due to a bad assumption that unmatched transactions should have no detail records. Leaving the comments until it's been fully tested for easy restoring. - Chris McGrath 10/13/2016
            ...
            if ( financialTransaction != null && authorizedPersonId.HasValue )
            {
                ...
                var hideAccounts = this.GetAttributeValue( "HideAccountControls" ).AsBoolean();
