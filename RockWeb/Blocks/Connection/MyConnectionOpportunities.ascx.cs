...
";
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                // Reset the state filter on every initial request to be Active, Future Follow Up and Future Follow Up (Past Due)
                rFilter.SaveUserPreference( "State", "State", "0;2;-2" );
        ...
        private void BindGrid()
        {
            ...
            if ( opportunitySummary != null )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    {
                        if ( sortProperty.Property == "LastActivityDate.CreatedDateTime" )
                        {
                            if ( sortProperty.Direction == SortDirection.Ascending )
                            {
                                requests = requests.OrderBy( r => r.ConnectionRequestActivities.OrderByDescending( a => a.CreatedDateTime ).Select( a => a.CreatedDateTime ).FirstOrDefault() );
                            }
                            else
                            {
                                requests = requests.OrderByDescending( r => r.ConnectionRequestActivities.OrderByDescending( a => a.CreatedDateTime ).Select( a => a.CreatedDateTime ).FirstOrDefault() );
                            }
                        }
                        else
                        {
                    ...
                    var mobilePhoneNumberDefinedValue = new DefinedValueService( rockContext ).Queryable().AsNoTracking().Where( d => d.Guid.ToString() == org.willowcreek.SystemGuid.DefinedValueGuids.MOBILE_PHONE ).FirstOrDefault();
                    ...
                    .Select( r => new
                    {
                        ...
                        Email = r.PersonAlias.Person.Email,
                        Mobile = r.PersonAlias.Person.PhoneNumbers.Where( p => p.NumberTypeValueId == mobilePhoneNumberDefinedValue.Id ).Select( p => p.NumberFormatted ).FirstOrDefault(),
                        Campus = r.Campus,
                        ...
                        LastActivityDate = r.ConnectionRequestActivities.OrderByDescending( a => a.CreatedDateTime ).Select( a => a.CreatedDateTime ).FirstOrDefault(),
