...
namespace RockWeb.Blocks.Connection
{
    ...
    public partial class ConnectionOpportunitySignup : RockBlock, IDetailBlock
    {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                if ( opportunity != null && defaultStatusId > 0 )
                {
                    ...
                    if ( person != null && person.PrimaryAliasId.HasValue )
                    {
                        ...
                        SaveBirthdate( bpBirthdate.SelectedDate, person, changes );
        ...
        private void SaveBirthdate( DateTime? birthdate, Person person, List<string> changes )
        {
            if ( birthdate.HasValue )
            {
                var oldBirthdate = person.BirthDate;
                var newBirthdate = birthdate;

                if( oldBirthdate.Value != newBirthdate.Value )
                {
                    person.SetBirthDate( birthdate );

                    History.EvaluateChange(changes, "Birthdate", oldBirthdate.Value, newBirthdate.Value );
                }
            }
        }
