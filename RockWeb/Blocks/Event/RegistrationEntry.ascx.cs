...
namespace RockWeb.Blocks.Event
{
    ...
    [BooleanField( "My Well ministry payment gatweay logo", "Display My Well ministry payment gateway logo?", false, "", 10, "myWellLogo" )]
    public partial class RegistrationEntry : RockBlock
    {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                else
                {
                    ...
                    if ( RegistrationTemplate != null )
                    {
                        ...
                        //If a signature form is required trigger redirection to Cudasing if browser is Safari so that the required cookie is set for the form to load.
                        if ( RegistrationTemplate.RequiredSignatureDocumentTemplateId.HasValue )
                        {
                            signatureFormRequired.Value = "True";
                        }
                        else
                        {
                            signatureFormRequired.Value = "False";
                        }
        ...
        protected void lbHowManyNext_Click( object sender, EventArgs e )
        {
            ...
            ProgressBarSteps = ( ( ( numHowMany.Value == 0 ) ? 1 : numHowMany.Value ) * registrantPages ) + ( Using3StepGateway ? 3 : 2 );
        ...
        protected void ddlFamilyMembers_SelectedIndexChanged( object sender, EventArgs e )
        {
            ...
            decimal currentStep = ( FormCount * CurrentRegistrantIndex + 1 ) + CurrentFormIndex;
        ...
        /// <summary>
        /// Handles the CheckedChanged event of the cbRegistrantDonorIsBusiness control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void cbRegistrantDonorIsBusiness_CheckedChanged( object sender, EventArgs e )
        {
            RegistrationState.DonorIsBusiness = ( ( RockCheckBox ) sender ).Checked;
            RegistrantDonor_HideAndShowBusinessFields();
        ...
        /// <summary>
        /// Hide or show registrant donor business fields.
        /// </summary>
        protected void RegistrantDonor_HideAndShowBusinessFields()
        {
            //Name
            var firstName = ( RockTextBox ) pnlRegistrantFields.FindControl( "tbFirstName" );

            var lastName = ( RockTextBox ) pnlRegistrantFields.FindControl( "tbLastName" );
            var lastNamePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextLastName" );
            var lastNamePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextLastName" );

            //Campus
            var campus = ( CampusPicker ) pnlRegistrantFields.FindControl( "cpHomeCampus" );
            var campusPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextCampus" );
            var campusPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextCampus" );

            //Address
            var address = ( AddressControl ) pnlRegistrantFields.FindControl( "acAddress" );
            var addressPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextAddress" );
            var addressPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextAddress" );

            //Email
            var email = ( EmailBox ) pnlRegistrantFields.FindControl( "tbEmail" );
            var emailPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextEmail" );
            var emailPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextEmail" );

            //Birthdate
            var birthdate = ( BirthdayPicker ) pnlRegistrantFields.FindControl( "bpBirthday" );
            var birthdatePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextBirthdate" );
            var birthdatePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextBirthdate" );

            //Anniversary
            var anniversary = ( BirthdayPicker ) pnlRegistrantFields.FindControl( "bpAnniversary" );
            var anniversaryPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextAnniversary" );
            var anniversaryPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextAnniversary" );

            //Grade
            var grade = ( GradePicker ) pnlRegistrantFields.FindControl( "gpGrade" );
            var gradePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextGrade" );
            var gradePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextGrade" );

            //Gender
            var gender = ( RockDropDownList ) pnlRegistrantFields.FindControl( "ddlGender" );
            var genderPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextGender" );
            var genderPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextGender" );

            //Marital Status
            var maritalStatus = ( RockDropDownList ) pnlRegistrantFields.FindControl( "ddlMaritalStatus" );
            var maritalStatusPreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextMaritalStatus" );
            var maritalStatusPostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextMaritalStatus" );

            //Mobile Phone
            var mobilePhone = ( PhoneNumberBox ) pnlRegistrantFields.FindControl( "ppMobile" );
            var mobilePhonePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextMobilePhone" );
            var mobilePhonePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextMobilePhone" );

            //Home Phone
            var homePhone = ( PhoneNumberBox ) pnlRegistrantFields.FindControl( "ppHome" );
            var homePhonePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextHomePhone" );
            var homePhonePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextHomePhone" );

            //Work Phone
            var workPhone = ( PhoneNumberBox ) pnlRegistrantFields.FindControl( "ppWork" );
            var workPhonePreText = ( LiteralControl ) pnlRegistrantFields.FindControl( "preTextWorkPhone" );
            var workPhonePostText = ( LiteralControl ) pnlRegistrantFields.FindControl( "postTextWorkPhone" );

            //Hide non business related fields (person fields)

            if ( RegistrationState.DonorIsBusiness )
            {

                bool visibility = false;
                if ( firstName != null )
                    firstName.Label = "Business Name";

                if ( lastName != null )
                    lastName.Visible = visibility;
                if ( lastNamePreText != null )
                    lastNamePreText.Visible = visibility;
                if ( lastNamePostText != null )
                    lastNamePostText.Visible = visibility;

                if ( lastName != null )
                    lastName.Text = string.Empty;

                if ( birthdate != null )
                    birthdate.Visible = visibility;
                if ( birthdatePreText != null )
                    birthdatePreText.Visible = visibility;
                if ( birthdatePostText != null )
                    birthdatePostText.Visible = visibility;

                if ( anniversary != null )
                    anniversary.Visible = visibility;
                if ( anniversaryPreText != null )
                    anniversaryPreText.Visible = visibility;
                if ( anniversaryPostText != null )
                    anniversaryPostText.Visible = visibility;

                if ( grade != null )
                    grade.Visible = visibility;
                if ( gradePreText != null )
                    gradePreText.Visible = visibility;
                if ( gradePostText != null )
                    gradePostText.Visible = visibility;

                if ( gender != null )
                    gender.Visible = visibility;
                if ( genderPreText != null )
                    genderPreText.Visible = visibility;
                if ( genderPostText != null )
                    genderPostText.Visible = visibility;

                if ( maritalStatus != null )
                    maritalStatus.Visible = visibility;
                if ( maritalStatusPreText != null )
                    maritalStatusPreText.Visible = visibility;
                if ( maritalStatusPostText != null )
                    maritalStatusPostText.Visible = visibility;

                if ( mobilePhone != null )
                    mobilePhone.Visible = visibility;
                if ( mobilePhonePreText != null )
                    mobilePhonePreText.Visible = visibility;
                if ( mobilePhonePostText != null )
                    mobilePhonePostText.Visible = visibility;

                if ( homePhone != null )
                    homePhone.Visible = visibility;
                if ( homePhonePreText != null )
                    homePhonePreText.Visible = visibility;
                if ( homePhonePostText != null )
                    homePhonePostText.Visible = visibility;
            }
            //Show person fields
            else
            {
        ...
        /// <summary>
        /// Handles the CheckChanged event of the cbRegistrarDonorIsBusiness control. 
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instnace containing the event data.</param>
        protected void cbRegistrarDonorIsBusiness_CheckChanged( object sender, EventArgs e )
        {
            RegistrationState.DonorIsBusiness = ( ( RockCheckBox ) sender ).Checked;
            //Hide Person Fields
            if ( RegistrationState.DonorIsBusiness )
            {
                tbYourFirstName.Label = "Business Name";
                tbYourLastName.Text = string.Empty;
                tbYourLastName.Visible = false;
                pnlRegistrarDateOfBirth.Visible = false;
            }
            //Show Person Fields
            else
            {
                tbYourFirstName.Label = "First Name";
                tbYourLastName.Visible = true;
                pnlRegistrarDateOfBirth.Visible = true;
            }
        ...
        private bool SetRegistrationState()
        {
            ...
            this.ProgressBarSteps = ( ( numHowMany.Value == 0 ) ? 1 : numHowMany.Value ) * registrantPages + 2;
                    ...
                    if ( registration != null )
                    {
                        ...
                        if ( hasPayment )
                        {
                            ...
                            if ( Using3StepGateway )
                            {
                                ...
                                {
                                    //If Payment fails to process, repeat Step 1 and Set Step2IFrameURL so that user can retry payment with a different credit card without having to navigate back or reload the form.
                                    //This creates a new token for the new transaction that will be sent to the payment gateway.
                                    string processStep1Error = string.Empty;
                                    if ( ProcessStep1( out processStep1Error ) )
                                    {
                                        if ( RegistrationTemplate != null && RegistrationTemplate.FinancialGateway != null )
                                        {
                                            var threeStepGateway = RegistrationTemplate.FinancialGateway.GetGatewayComponent() as ThreeStepGatewayComponent;
                                    ...
                                    //Build error messages and throw the exception.
                                    StringBuilder errorMessages = new StringBuilder();
                                    errorMessages.AppendLine( errorMessage );
                                    errorMessages.AppendLine( processStep1Error );
            ...
            try
            {
                ...
                if ( newRegistration != null )
                {
                    if ( isNewRegistration )
                    {
                        ...
                        newRegistration.LaunchWorkflow( RegistrationTemplate.RegistrationWorkflowTypeId, newRegistration.ToString() + "(R_ID: " + newRegistration.Id.ToString() + ")" );
                        newRegistration.LaunchWorkflow( RegistrationInstanceState.RegistrationWorkflowTypeId, newRegistration.ToString() + "(R_ID: " + newRegistration.Id.ToString() + ")" );
                    }
        ...
        private Registration SaveRegistration( RockContext rockContext, bool hasPayment )
        {
            ...
            if ( !registration.PersonAliasId.HasValue )
            {
                ...
                if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionRegistration )
                {
                    Location address = new Location();
                    if ( cbRegistrarDonorIsBusiness.Checked )
                    {
                        person.FirstName = null;
                        person.MiddleName = null;
                        person.LastName = tbYourFirstName.Text;
                        person.NickName = null;
                        person.RecordTypeValueId = 2;
                    }
                    else
                    {
                        person.SetBirthDate( bpDOB.SelectedDate );
                    }
                    address.Country = acYourAddress.Country;
                    address.Street1 = acYourAddress.Street1;
                    address.Street2 = acYourAddress.Street2;
                    address.City = acYourAddress.City;
                    address.State = acYourAddress.State;
                    address.PostalCode = acYourAddress.PostalCode;

                    registrar = SavePerson( rockContext, person, RegistrationState.FamilyGuid, CampusId, address, adultRoleId, childRoleId, multipleFamilyGroupIds, ref singleFamilyId );
                ...
                else if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
                {
                    //Save registrar later when saving registrant as it should be the same person.
                }
                else if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.EventRegistration )
                {
            ...
            try
            {
                ...
                foreach ( var registrantInfo in RegistrationState.Registrants.ToList() )
                {
                            ...
                            .Where( t => t.FieldSource == RegistrationFieldSource.PersonField ) ) )
                    {
                        ...
                        if ( fieldValue != null )
                        {
                            switch ( field.PersonFieldType )
                            {
                                ...
                                case RegistrationPersonFieldType.Anniversary:
                                    {
                                        var newAnniversaryDate = fieldValue.ToString().AsDateTime();
                                        History.EvaluateChange( personChanges, "Anniversary Date", person.AnniversaryDate, newAnniversaryDate );
                                        person.AnniversaryDate = newAnniversaryDate;
                                        break;
                                    }
                    ...
                    if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
                    {
                        if ( RegistrationState.DonorIsBusiness )
                        {
                            person.FirstName = null;
                        ...
                        //Save registrant and set registrar as the same person.
                        registrar = SavePerson( rockContext, person, registrantInfo.FamilyGuid, campusId, location, adultRoleId, childRoleId, multipleFamilyGroupIds, ref singleFamilyId );
            ...
            Dictionary<Guid, int> multipleFamilyGroupIds, ref int? singleFamilyId )
        {
            ...
            if ( familyId.HasValue && location != null )
            {
                ...
                if ( homeLocationType != null && workLocationType != null )
                {
                    ...
                    if ( familyGroup != null )
                    {
                        ...
                        if ( RegistrationState.DonorIsBusiness )
                        {
                            locationType = Rock.SystemGuid.DefinedValue.GROUP_LOCATION_TYPE_WORK;
                        }
                        else
                        {
                            locationType = Rock.SystemGuid.DefinedValue.GROUP_LOCATION_TYPE_HOME;
                        }
                        ...
                        GroupService.AddNewGroupAddress(
                            ...
                            familyGroup,
                            locationType,
                            location.Street1, location.Street2, location.City, location.State, location.PostalCode, location.Country, true, "Event Registration - " + RegistrationInstanceState.Name );
                    }
                }
            }
        ...
        private bool SaveTransaction( GatewayComponent gateway, Registration registration, FinancialTransaction transaction, PaymentInfo paymentInfo, RockContext rockContext )
        {
            if ( transaction != null )
            {
                ...
                txnChanges.Add( "Name On Credit Card: " + ( gateway != null && gateway.SplitNameOnCard ? txtCardFirstName.Text + " " + txtCardLastName.Text : txtCardName.Text ) );

                History.EvaluateChange( txnChanges, "Transaction Code", string.Empty, transaction.TransactionCode );

                if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.EventRegistration )
                {
                    transaction.AuthorizedPersonAliasId = registration.PersonAliasId;
                }
                else if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
                {
                    transaction.AuthorizedPersonAliasId = registration.Registrants.FirstOrDefault().PersonAliasId;
                }
                else if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionRegistration )
                {
                    transaction.AuthorizedPersonAliasId = registration.PersonAliasId;
                }
                ...
                DefinedValueCache txnType;
                if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionRegistration || RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
                {
                    txnType = DefinedValueCache.Read( new Guid( Rock.SystemGuid.DefinedValue.TRANSACTION_TYPE_CONTRIBUTION ) );
                }
                else
                {
                    txnType = DefinedValueCache.Read( new Guid( Rock.SystemGuid.DefinedValue.TRANSACTION_TYPE_EVENT_REGISTRATION ) );
                }
                ...
                rockContext.WrapTransaction( () =>
                {
                    ...
                    batchPrefix = batchPrefix.Substring( 0, Math.Min( batchPrefix.Length, 36 ) );
                    batchPrefix += " - Credit Card";
        ...
        private void ShowRegistrant()
        {
            if ( RegistrationState != null && RegistrationState.RegistrantCount > 0 )
            {
                ...
                if ( registrant != null )
                {
                    ...
                    nbType.Text = registrant.OnWaitList ? string.Format( "<span style=\"color: red\"><b><font size=\"+1\">This event is currently at full capacity. You will be placed on a waitlist and notified when a spot becomes available.</font></b></span>", RegistrantTerm.ToLower() ) : string.Format( "This {0} will be fully registered.", RegistrantTerm.ToLower() );
        ...
        private void ShowPayment()
        {
                ...
                RegistrationTemplate.FinancialGateway != null )
            {
                ...
                if ( component != null )
                {
                    ...
                    if ( component is Rock.NMI.Gateway )
                    {
                        lAmountPaidConfirmation.Visible = true;
                        lAmountPaidConfirmation.Text = "$ " + ( RegistrationState.PaymentAmount ?? 0.0m ).ToString( "N2" );
                    }

                    if ( GetAttributeValue( "myWellLogo" ).AsBoolean() && ( component is Rock.NMI.Gateway ) )
                    {
                        myWellLogo.Visible = true;
                    }
        ...
        /// <summary>
        /// Sets the contributor values.
        /// </summary>
        private void SetContributorValues()
        {
            setSummaryText();

            if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.EventRegistration )
            {
                //Set Registration Completed By Term
                lRegistrationCompletedByTerm.Text = "Completed";                
            }

            if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
            {
                //Set Registration Completed By Term
                lRegistrationCompletedByTerm.Text = "Made";

                //Hide discount code
                if ( divDiscountCode.Visible )
                {
                    divDiscountCode.Visible = false;
                }

                if ( pnlRegistrarInfo.Visible )
                {
                    //Load registrar values 
                    foreach ( var registrantField in RegistrationState.Registrants[0].FieldValues )
                    {
                        if ( ( RegistrationFieldSource ) registrantField.Value.FieldSource == RegistrationFieldSource.PersonField )
                        {
                            switch ( registrantField.Value.PersonFieldType )
                            {
                                case RegistrationPersonFieldType.FirstName:
                                    {
                                        tbYourFirstName.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                        tbYourFirstName.Enabled = false;
                                        if ( RegistrationState.DonorIsBusiness )
                                        {
                                            tbYourFirstName.Label = "Business Name";
                                            tbYourLastName.Visible = false;
                                        ...
                                        {
                                            tbYourFirstName.Label = "FirstName";
                                        }
                                        break;
                                    }
                                case RegistrationPersonFieldType.LastName:
                                    {
                                        ...
                                        {
                                            tbYourLastName.Visible = true;
                                            tbYourLastName.Label = "Last Name";
                                            tbYourLastName.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                            tbYourLastName.Enabled = false;
                                        }
                                ...
                                case RegistrationPersonFieldType.Email:
                                    {
                                        tbConfirmationEmail.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                        tbConfirmationEmail.Enabled = false;
                ...
                if ( pnlPaymentInfo.Visible )
                {
                    //Hide Last Name on Card when donor is a business
                    if ( RegistrationState.DonorIsBusiness )
                    {
                        txtCardLastName.Visible = false;
                        txtCardLastName.Text = string.Empty;
                        txtCardName.Label = "Business Name on Card";
                        txtCardFirstName.Label = "Business Name on Card";
                    }
                    else
                    {
                        txtCardLastName.Visible = txtCardFirstName.Visible;
                        txtCardName.Label = "Name on Card";
                        txtCardFirstName.Label = "First Name on Card";
                    }

                    //Set Card Name, Card First Name, Card Last Name and Address
                    foreach ( var registrantField in RegistrationState.Registrants[0].FieldValues )
                    {
                        if ( ( RegistrationFieldSource ) registrantField.Value.FieldSource == RegistrationFieldSource.PersonField )
                        {
                            switch ( registrantField.Value.PersonFieldType )
                            {
                                case RegistrationPersonFieldType.FirstName:
                                    {
                                        txtCardName.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                        txtCardName.Enabled = false;

                                        txtCardFirstName.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                        txtCardFirstName.Enabled = false;
                                        break;
                                    }
                                case RegistrationPersonFieldType.LastName:
                                    {
                                        txtCardLastName.Text = registrantField.Value.FieldValue != null ? ( string ) registrantField.Value.FieldValue : string.Empty;
                                        txtCardLastName.Enabled = false;
                                ...
                                case RegistrationPersonFieldType.Address:
                                    {
                                        Location address = registrantField.Value.FieldValue != null ? ( Location ) registrantField.Value.FieldValue : new Location();
                                        if ( address != null && acBillingAddress != null )
                                        {
                                            acBillingAddress.Country = address.Country;
                                            acBillingAddress.Street1 = address.Street1;
                                            acBillingAddress.Street2 = address.Street2;
                                            acBillingAddress.City = address.City;
                                            acBillingAddress.State = address.State;
                                            acBillingAddress.PostalCode = address.PostalCode;
                                            acBillingAddress.Enabled = false;
                                        }
                                        break;
                                    }
                            }
            ...
            if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionRegistration )
            {
                //Set Registration Completed By Term
                lRegistrationCompletedByTerm.Text = "Completed";
                //pnlRegistrarDonorIsBusiness.Visible = true;
                pnlRegistrarDonorAddress.Visible = true;

                if ( pnlPaymentInfo.Visible )
                {
                    //Set Card Name, Card First Name, Card Last Name and Address
                    txtCardName.Text = tbYourFirstName.Text;
                    txtCardName.Enabled = false;

                    txtCardFirstName.Text = tbYourFirstName.Text;
                    txtCardFirstName.Enabled = false;
                    ...
                    else
                    {
                        ...
                        txtCardName.Label = "Name on Card";
                        txtCardFirstName.Label = "First Name on Card";
                        txtCardLastName.Text = tbYourLastName.Text;
                    }
                    txtCardLastName.Enabled = false;

                    if ( acYourAddress != null && acBillingAddress != null )
                    {
                        acBillingAddress.Country = acYourAddress.Country;
                        acBillingAddress.Street1 = acYourAddress.Street1;
                        acBillingAddress.Street2 = acYourAddress.Street2;
                        acBillingAddress.City = acYourAddress.City;
                        acBillingAddress.State = acYourAddress.State;
                        acBillingAddress.PostalCode = acYourAddress.PostalCode;
                        acBillingAddress.Enabled = false;
                    }
                }
            }
        ...
        /// <summary>
        /// Displays the Summary Text message.
        /// </summary>
        ...
        public void setSummaryText()
        {
            try
            {
                var mergeFields = new Dictionary<string, object>();
                mergeFields.Add( "CurrentPerson", CurrentPerson );
                mergeFields.Add( "RegistrationInstance", RegistrationInstanceState );
                mergeFields.Add( "Registration", RegistrationInstanceState.Registrations );

                lSummaryText.Text = RegistrationInstanceState.RegistrationTemplate.SummaryText.ResolveMergeFields( mergeFields );

                // show debug info
                if ( GetAttributeValue( "EnableDebug" ).AsBoolean() && UserCanEdit )
                {
                    lSummaryTextDebug.Visible = true;
                    lSummaryTextDebug.Text = mergeFields.lavaDebugInfo();
                }
            }
            catch ( Exception ex )
            {
...
",
        ...
        private void CreateRegistrantControls( bool setValues )
        {
            ...
            if ( FormCount > CurrentFormIndex )
            {
                    ...
                    .OrderBy( f => f.Order ) )
                {
                    ...
                    if ( field.Attribute != null && field.AttributeId.HasValue == true)
                    {
                        value = AttributeCache.Read( field.AttributeId.Value ).DefaultValue;
                    }
                    ...
                    //If Registration Template Type is Fundraising, add Is Donor a business option
                    if ( RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising && field.FieldSource == RegistrationFieldSource.PersonField && field.PersonFieldType == RegistrationPersonFieldType.FirstName )
                    {
                        var cbDonorIsBusiness = new RockCheckBox();
                        cbDonorIsBusiness.ID = "cbRegistrantDonorIsBusiness";
                        cbDonorIsBusiness.Label = "Is this donation being made on behalf of a Business?";
                        cbDonorIsBusiness.Text = "Yes";
                        cbDonorIsBusiness.Help = "Check this box if the donation is being made on behalf of a Business, otherwise leave blank.";
                        cbDonorIsBusiness.AutoPostBack = true;
                        cbDonorIsBusiness.Required = false;
                        cbDonorIsBusiness.RequiredErrorMessage = "Answer to donating on behalf of a business is required.";
                        cbDonorIsBusiness.Checked = RegistrationState.DonorIsBusiness;
                        cbDonorIsBusiness.CheckedChanged += new EventHandler( cbRegistrantDonorIsBusiness_CheckedChanged );
                        phRegistrantControls.Controls.Add( cbDonorIsBusiness );
                    }

                    if ( !string.IsNullOrWhiteSpace( field.PreText ) )
                    {
                        CreatePrePostTextFields( field, true );
                    }
                    ...
                    {
                        CreatePrePostTextFields( field, false );
                    }
        ...
        private void CreatePersonField( RegistrationTemplateFormField field, bool setValue, object fieldValue, bool familyMemberSelected )
        {
            ...
            switch ( field.PersonFieldType )
            {
                ...
                case RegistrationPersonFieldType.Anniversary:
                    {
                        var bpAnniversary = new BirthdayPicker();
                        bpAnniversary.ID = "bpAnniversary";
                        bpAnniversary.Label = "Anniversary";
                        bpAnniversary.Required = field.IsRequired;
                        bpAnniversary.ValidationGroup = BlockValidationGroup;
                        phRegistrantControls.Controls.Add( bpAnniversary );

                        if ( setValue && fieldValue != null )
                        {
                            var value = fieldValue as DateTime?;
                            bpAnniversary.SelectedDate = value;
                        }
                        break;
                    }
        ...
        private void CreatePrePostTextFields( RegistrationTemplateFormField field, bool isPretext )
        {
            string prePostText = isPretext ? field.PreText : field.PostText;

            if ( field.FieldSource == RegistrationFieldSource.PersonField )
            {
                string preTextPostTextControlID = string.Empty;

                switch ( field.PersonFieldType )
                {
                    case RegistrationPersonFieldType.FirstName:
                        {
                            preTextPostTextControlID = isPretext ? "preTextFirstName" : "postTextFirstName";
                            break;
                        }

                    case RegistrationPersonFieldType.LastName:
                        {
                            preTextPostTextControlID = isPretext ? "preTextLastName" : "postTextLasttName";
                            break;
                        }

                    case RegistrationPersonFieldType.Campus:
                        {
                            preTextPostTextControlID = isPretext ? "preTextCampus" : "postTextCampus";
                            break;
                        }

                    case RegistrationPersonFieldType.Address:
                        {
                            preTextPostTextControlID = isPretext ? "preTextAddress" : "postTextAddress";
                            break;
                        }

                    case RegistrationPersonFieldType.Email:
                        {
                            preTextPostTextControlID = isPretext ? "preTextEmail" : "postTextEmail";
                            break;
                        }

                    case RegistrationPersonFieldType.Birthdate:
                        {
                            preTextPostTextControlID = isPretext ? "preTextBirthdate" : "postTextBirthdate";
                            break;
                        }

                    case RegistrationPersonFieldType.Anniversary:
                        {
                            preTextPostTextControlID = isPretext ? "preTextAnniversary" : "postTextAnniversary";
                            break;
                        }

                    case RegistrationPersonFieldType.Grade:
                        {
                            preTextPostTextControlID = isPretext ? "preTextGrade" : "postTextGrade";
                            break;
                        }

                    case RegistrationPersonFieldType.Gender:
                        {
                            preTextPostTextControlID = isPretext ? "preTextGender" : "postTextGender";
                            break;
                        }

                    case RegistrationPersonFieldType.MaritalStatus:
                        {
                            preTextPostTextControlID = isPretext ? "preTextMaritalStatus" : "postTextMaritalStatus";
                            break;
                        }

                    case RegistrationPersonFieldType.MobilePhone:
                        {
                            preTextPostTextControlID = isPretext ? "preTextMobilePhone" : "postTextMobilePhone";
                            break;
                        }
                    case RegistrationPersonFieldType.HomePhone:
                        {
                            preTextPostTextControlID = isPretext ? "preTextHomePhone" : "postTextHomePhone";
                            break;
                        }

                    case RegistrationPersonFieldType.WorkPhone:
                        {
                            preTextPostTextControlID = isPretext ? "preTextWorkPhone" : "postTextWorkPhone";
                            break;
                        }
                }
                var lControl = new LiteralControl();
                lControl.ID = preTextPostTextControlID;
                lControl.Text = prePostText;
                phRegistrantControls.Controls.Add( lControl );
            }
            else
            {
                phRegistrantControls.Controls.Add( new LiteralControl( prePostText ) );
            }
        }
        ...
        private object ParsePersonField( RegistrationTemplateFormField field )
        {
            switch ( field.PersonFieldType )
            {
                ...
                case RegistrationPersonFieldType.Anniversary:
                    {
                        var bpAnniversary = phRegistrantControls.FindControl( "bpAnniversary" ) as BirthdayPicker;
                        return bpAnniversary != null ? bpAnniversary.SelectedDate : null;
                    }
        ...
        private void CreateSummaryControls( bool setValues )
        {
            ...
            if ( setValues && RegistrationState != null && RegistrationInstanceState != null )
            {
                ...
                if ( costs.Where( c => c.Cost > 0.0M ).Any() )
                {
                    ...
                    {
                        //Set up minimum payment required by default. 
                        if ( minimumPayment.HasValue )
                        {
                            if ( balanceDue > minimumPayment.Value )
                            {
                                RegistrationState.PaymentAmount = minimumPayment.Value;
                            }
                            else
                            {
                    ...
                    if ( balanceDue > 0 && RegistrationTemplate.FinancialGateway != null )
                    {
                        ...
                        if ( component != null )
                        {
                            ...
                            if ( Using3StepGateway )
                            {
                                ...
                                lAmountPaidConfirmation.Visible = false;
                                myWellLogo.Visible = false;
