...
{% if AdditionalFieldsNeeded or RegistrationInstance.RegistrationTemplate.Fees != empty or Registration.BalanceDue > 0 %}
    <p>
        <strong>Additional information is needed{% if Registration.BalanceDue > 0 %} and a balance of {{ currencySymbol }}{{ Registration.BalanceDue | Format:'#,##0.00' }} remains on this regsitration{% endif %}. Please visit the 
...
";
        ...
        protected void btnEdit_Click( object sender, EventArgs e )
        {
            ...
            RegistrationTemplateType_HideOrShowControls();
        }
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
            ...
            RegistrationTemplate.RegistrationTemplateType = ddlRegistrationTemplateType.SelectedValueAsEnum<RegistrationTemplateType>();
            ...
            RegistrationTemplate.SummaryText = ceSummaryText.Text;
        ...
        /// <summary>
        /// Handles the CheckedChanged event of the cbRegistrationTemplateType control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void ddlRegistrationTemplateType_SelectedIndexChanged( object sender, EventArgs e )
        {
            RegistrationTemplateType_HideOrShowControls();
        }

        /// <summary>
        /// Hides or shows controls specific to the registration template type selected
        /// </summary>
        protected void RegistrationTemplateType_HideOrShowControls()
        {
            ParseControls();

            if ( ddlRegistrationTemplateType.SelectedValueAsEnum<RegistrationTemplateType>() == RegistrationTemplateType.ContributionRegistration || ddlRegistrationTemplateType.SelectedValueAsEnum<RegistrationTemplateType>() == RegistrationTemplateType.ContributionFundraising )
            {
                //Disable and hide Multiple Registrants Option
                cbMultipleRegistrants.Checked = false;
                cbMultipleRegistrants.Visible = false;
                nbMaxRegistrants.Text = string.Empty;
                nbMaxRegistrants.Visible = false;

                //Hide and set Registrants In Same Family to No
                rblRegistrantsInSameFamily.SetValue( RegistrantsSameFamily.No.ConvertToInt() );
                rblRegistrantsInSameFamily.Visible = false;

                //Disable and hide Enable Wait List
                cbWaitListEnabled.Checked = false;
                cbWaitListEnabled.Visible = false;

            }
            else if ( ddlRegistrationTemplateType.SelectedValueAsEnum<RegistrationTemplateType>() == RegistrationTemplateType.EventRegistration )
            {
                //Show Multiple Registrants Option
                cbMultipleRegistrants.Visible = true;

                //Show Registrants In Same Family
                rblRegistrantsInSameFamily.Visible = true;

                //Show Enable Wait List
                cbWaitListEnabled.Visible = true;
            }
...
";
                ...
                // Add Email Address field if it doesn't exist
                if ( !defaultForm.Fields
                    .Any( f =>
                        f.FieldSource == RegistrationFieldSource.PersonField &&
                        f.PersonFieldType == RegistrationPersonFieldType.Email ) )
                {
                    var formField = new RegistrationTemplateFormField();
                    formField.FieldSource = RegistrationFieldSource.PersonField;
                    formField.PersonFieldType = RegistrationPersonFieldType.Email;
                    formField.IsGridField = true;
                    formField.IsRequired = true;
                    formField.Order = defaultForm.Fields.Any() ? defaultForm.Fields.Max( f => f.Order ) + 1 : 0;
                    defaultForm.Fields.Add( formField );
                ...
                if ( !defaultForm.Fields
                    .Any( f =>
                        f.FieldSource == RegistrationFieldSource.PersonField &&
                        f.PersonFieldType == RegistrationPersonFieldType.Birthdate ) )
                {
                    var formField = new RegistrationTemplateFormField();
                    formField.FieldSource = RegistrationFieldSource.PersonField;
                    formField.PersonFieldType = RegistrationPersonFieldType.Birthdate;
                    formField.IsGridField = true;
                    formField.IsRequired = true;
                    formField.Order = defaultForm.Fields.Any() ? defaultForm.Fields.Max( f => f.Order ) + 1 : 0;
                    defaultForm.Fields.Add( formField );
                ...
                // Add mobile phone field if it doesn't exist
                if ( !defaultForm.Fields
                    .Any( f =>
                        f.FieldSource == RegistrationFieldSource.PersonField &&
                        f.PersonFieldType == RegistrationPersonFieldType.MobilePhone ) )
                {
                    var formField = new RegistrationTemplateFormField();
                    formField.FieldSource = RegistrationFieldSource.PersonField;
                    formField.PersonFieldType = RegistrationPersonFieldType.MobilePhone;
                    formField.IsGridField = false;
        ...
        private void ShowEditDetails( RegistrationTemplate RegistrationTemplate, RockContext rockContext )
        {
            ...
            ddlRegistrationTemplateType.SetValue( RegistrationTemplate.RegistrationTemplateType.ConvertToInt() );
            ...
            ceSummaryText.Text = RegistrationTemplate.SummaryText;
...
", formFieldName, fieldTypeName, formField.FieldSource.ConvertToString() );
        ...
        {
            ddlRegistrationTemplateType.BindToEnum<RegistrationTemplateType>();
