...
namespace RockWeb.Blocks.Event
{
    ...
    public partial class RegistrationDetail : RockBlock, IDetailBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            {
                loadPaymentAttributes();
            }

            cbPaymentAmount.Focus();
        }
            ...
            if ( ppPerson.PersonId.HasValue )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    if ( person != null )
                    {
                        ...
                        {
                            //If its a business set business name on registered by first name when blank.
                            if ( person.RecordTypeValueId.HasValue && !( person.RecordTypeValueId.Value == 2 ) )
                            {
                                tbFirstName.Text = person.NickName;
                            }
                            else
                            {
                                tbFirstName.Text = person.LastName;
                            }
                        }
        ...
        protected void ddlCurrencyType_SelectedIndexChanged( object sender, EventArgs e )
        {
            ...
            var checkCurrencyType = DefinedValueCache.Read( Rock.SystemGuid.DefinedValue.CURRENCY_TYPE_CHECK );
            if(currencyType.HasValue && currencyType.Value == checkCurrencyType.Id)
            {
                tbTransactionCode.Focus();
            }

            loadPaymentAttributes();
        }

        private void loadPaymentAttributes()
        {
...
";
        ...
        private bool ProcessPayment( bool submitToGateway, RockContext rockContext, Registration registration, int? personAliasId, decimal amount, out string errorMessage )
        {
            ...
            else
            {
                ...
                transaction.FinancialPaymentDetail.LoadAttributes();
                Helper.GetEditValues( phPaymentAttributeEdits, transaction.FinancialPaymentDetail );
            ...
            if ( transaction != null )
            {
                ...
                if ( this.Registration.RegistrationInstance.RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionRegistration || this.Registration.RegistrationInstance.RegistrationTemplate.RegistrationTemplateType == RegistrationTemplateType.ContributionFundraising )
                {
                ...
                batchPrefix.Substring( 0, Math.Min( batchPrefix.Length, 41 ) );
                batchPrefix += " - Scan";
                ...
                transaction.FinancialPaymentDetail.SaveAttributeValues( rockContext );
