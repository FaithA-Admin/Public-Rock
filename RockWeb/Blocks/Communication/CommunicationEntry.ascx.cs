...
";
        ...
        private Rock.Model.Communication UpdateCommunication(RockContext rockContext)
        {
            ...
            if ( cbUniqueEmailPhone.Checked )
            {
                //Communication is Email
                if(communication.MediumEntityTypeId == 37 )
                {
                    
                    var originalRecipientsPersonAliasIds = communication.Recipients.Select(r => r.PersonAlias.Id);
                    var personAlias = new PersonAliasService( rockContext ).Queryable();
                    var person = new PersonService( rockContext ).Queryable();


                    var uniqueRecipients = from pa in personAlias
                                                         join p in person on pa.PersonId equals p.Id
                                                         where originalRecipientsPersonAliasIds.Contains(pa.Id) && p.Email != null && p.Email != string.Empty
                                                         select new { pa.Id, p.Email, p.CreatedDateTime };
                    var uniqueRecipientsPersonAliasIds = uniqueRecipients
                                                        .GroupBy( row => row.Email )
                                                        .Select( group => group.OrderByDescending( r => r.CreatedDateTime ).FirstOrDefault() )
                                                        .ToList()
                                                        .Select( s=> s.Id );
                    
                    communication.Recipients = communication.Recipients.Where( w => uniqueRecipientsPersonAliasIds.Contains( w.PersonAlias.Id ) ).ToList();
    
                }
                //Communication is SMS
                else if(communication.MediumEntityTypeId == 38 )
                {
                    var originalRecipientsPersonAliasIds = communication.Recipients.Select( r => r.PersonAlias.Id );
                    var personAlias = new PersonAliasService( rockContext ).Queryable();
                    var person = new PersonService( rockContext ).Queryable();
                    var mobile = new PhoneNumberService( rockContext ).Queryable();
                    

                    var uniqueRecipients = from pa in personAlias
                                           join p in person on pa.PersonId equals p.Id
                                           join m in mobile on p.Id equals m.PersonId
                                           where originalRecipientsPersonAliasIds.Contains( pa.Id ) && m.IsMessagingEnabled == true
                                           select new { pa.Id, m.Number, m.CreatedDateTime };
                    var uniqueRecipientsPersonAliasIds = uniqueRecipients
