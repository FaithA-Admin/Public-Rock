            ...
            if ( rockBlock != null )
            {
                ...
                foreach ( var userPreference in rockBlock.GetUserPreferences( keyPrefix ) )
                {
                    ...
                    if ( blockKey.Contains("|") && keyName.Length == 2 )
                    {
                        ...
                        if ( !_userPreferences.Any( a => (a.Key == keyName[0] && a.Name == keyName[1] ) ))
                        {
...
";
        ...
        /// <summary>
        /// Gets the user preference for a given key and name if it exists
        /// </summary>
        /// <param name="key">The key.</param>
        /// <param name="name">The name.</param>
        /// <returns></returns>
        public string GetUserPreference( string key, string name )
        {
            return _userPreferences.Where( p => ( p.Key == key && p.Name == name ) ).Select( p => p.Value ).FirstOrDefault() ?? string.Empty;
        }
