...
namespace Rock.Web.UI.Controls
{
    ...
    public class GridActions : CompositeControl
    {
        ...
        public HtmlGenericControl NewAddButton()
        {
            var aAdd = new HtmlGenericControl( "a" );
            aAdd.ID = "aAdd";
        ...
        public LinkButton NewAddLinkButton()
        {
            var lbAdd = new LinkButton();
            lbAdd.ID = "lbAdd";
        ...
        public void CopyAddButton(ControlCollection controls, Page page)
        {
            if ( ShowAdd )
            {
                if ( string.IsNullOrWhiteSpace( ClientAddScript ) )
                {
                    var lbAdd = NewAddLinkButton();
                    
                    // make sure delete button is registered for async postback (needed just in case the grid was created at runtime)
                    var sm = ScriptManager.GetCurrent( page );
                    sm.RegisterAsyncPostBackControl( lbAdd );
                    
                    controls.Add( lbAdd );
                }
                else
                {
                    controls.Add( NewAddButton() );
                }
            }
        }
