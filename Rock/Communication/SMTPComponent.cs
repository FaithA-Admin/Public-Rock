                            ...
                            if ( recipient != null )
                            {
                                ...
                                else
                                {
                                    try
                                    {
                                        ...
                                        metaData.Add( "communication_subject", message.Subject );
                                        metaData.Add( "communication_sender", fromAddress );
                                        metaData.Add( "communication_recipientName", recipient.PersonAlias.Person.FullName );
                ...
                using ( var smtpClient = GetSmtpClient() )
                {
                    foreach ( var recipientData in recipients )
                    {
                        ...
                        foreach ( string to in sendTo )
                        {
                            ...
                            {
                                recipientName = String.Join( ", ", new PersonService( rockContext ).Queryable().Where( w => w.Email == to ).ToList().Select( s => s.FullName ).Distinct().ToArray() );
                            }
                            var metaData = new Dictionary<string, string>();
                            metaData.Add( "communication_subject", message.Subject );
                            metaData.Add( "communication_sender", from );
                            metaData.Add( "communication_recipientName", recipientName );
                            AddAdditionalHeaders( message, metaData );
        ...
        public void Send( Dictionary<string, string> mediumData, List<string> recipients, string appRoot, string themeRoot, bool createCommunicationHistory, Dictionary<string, string> metaData )
        {
            try
            {
                ...
                if ( !string.IsNullOrWhiteSpace( from ) )
                {
                    ...
                    using ( var rockContext = new RockContext() )
                    {
                        recipientName = String.Join( ", ", new PersonService( rockContext ).Queryable().Where( w => recipients.Contains( w.Email ) ).ToList().Select( s => s.FullName ).Distinct().ToArray() );
                    }
                    if ( metaData == null )
                    {
        ...
        public void Send( List<string> recipients, string from, string fromName, string subject, string body, string appRoot, string themeRoot, List<Attachment> attachments, bool createCommunicationHistory )
        {
            try
            {
                ...
                if ( !string.IsNullOrWhiteSpace( from ) )
                {
                    ...
                    var metaData = new Dictionary<string, string>();
                    metaData.Add( "communication_subject", message.Subject );
                    metaData.Add( "communication_sender", msgFrom );
                    metaData.Add( "communication_recipientName", recipientName );
                    AddAdditionalHeaders( message, metaData );
