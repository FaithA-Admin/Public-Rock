...
namespace Rock.Model
{
    ...
    public partial class AttendanceCodeService
    {
        ...
        public static AttendanceCode GetNew( int codeLength )
        {
            return GetNew( codeLength, char.MinValue );
        }
        ...
        /// </summary>
        /// <param name="codeLength">A <see cref="System.Int32"/> representing the length of the (security) code.</param>
        /// <param name="familyLastNameInitial">A <see cref="System.Char"/> representing the family name first initial.</param>
        /// <returns>A new <see cref="Rock.Model.AttendanceCode"/></returns>
        public static AttendanceCode GetNew( int codeLength, char familyLastNameInitial )
        {
            lock ( _obj )
            {
                using ( var rockContext = new Rock.Data.RockContext() )
                {
                    ...
                    string code = GenerateRandomCode( codeLength, familyLastNameInitial );
                    while ( noGood.Any( s => s == code ) || _todaysCodes.Any( c => c == code ) )
                    {
                        code = GenerateRandomCode( codeLength, familyLastNameInitial );
                    }
        ...
        private static string GenerateRandomCode( int length, char familyLastNameInitial )
        {
            StringBuilder sb = new StringBuilder();

            int poolSize = codeCharacters.Length;

            if( familyLastNameInitial != char.MinValue )
            {
                sb.Append( familyLastNameInitial.ToString().ToUpper() );
