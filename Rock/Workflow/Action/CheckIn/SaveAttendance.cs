    ...
    public class SaveAttendance : CheckInActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, Model.WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            if ( checkInState != null )
            {
                ...
                Guid appendFamilyNameInitialToSecurityCodeGuid = "7B46BD48-4579-40C8-A56A-F37C996EC38B".AsGuid();
                var appendFamilyNameInitialToSecurityCodeAttributeId = new AttributeService( rockContext ).Queryable().Where( k => k.Guid == appendFamilyNameInitialToSecurityCodeGuid ).Select( k => k.Id ).FirstOrDefault().ToString().AsIntegerOrNull();
                var appendFamilyNameInitialToSecurityCodeAttributeValue = new AttributeValueService( rockContext ).Queryable().Where( i => i.AttributeId == appendFamilyNameInitialToSecurityCodeAttributeId && i.EntityId == checkInState.CheckinTypeId ).Select( v => v.Value ).FirstOrDefault();
                bool appendFamilyNameInitialToSecurityCode = false;
                if( appendFamilyNameInitialToSecurityCodeAttributeValue != null)
                {
                    appendFamilyNameInitialToSecurityCode = appendFamilyNameInitialToSecurityCodeAttributeValue.AsBoolean();
                }
                ...
                if ( family != null )
                {
                    foreach ( var person in family.GetPeople( true ) )
                    {
                        ...
                        {
                            if ( appendFamilyNameInitialToSecurityCode )
                            {
                                attendanceCode = AttendanceCodeService.GetNew( securityCodeLength, family.Group.Name[0] );
                            }
                            else
                            {
